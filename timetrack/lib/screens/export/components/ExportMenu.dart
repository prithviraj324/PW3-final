...
import 'package:TimeTrack/blocs/timers/timers_bloc.dart';
      ...
      onSelected: (ExportMenuItem item) async {
        switch (item) {
          ...
          case ExportMenuItem.export:
            ...
            var dbPath = p.join(databasesPath, 'TimeTrack.db');
            ...
            try {
              ...
              if (Platform.isAndroid) {
                ...
                File copiedDB = await File(dbPath)
                    .copy(p.join(directory.path, "TimeTrack.db"));
              ...
              await Share.shareFiles(<String>[dbPath],
                  ...
                  subject: L10N
                      .of(context)
                      .tr
                      .TimeTrackDatabase(dateFormat.format(DateTime.now())));
            } on Exception catch (e) {
              scaffoldKey.currentState.showSnackBar(SnackBar(
                backgroundColor: Theme.of(context).errorColor,
                content: Text(
